...
class Lattice:
    ...
    @property
    def dimension(self:'Lattice') -> int:
        return 0
...
class Direction(Enum):
    ...
    def isSameDirection(self: 'Direction', dire : 'Direction') -> bool:
        return (self.isHorizontal and dire.isHorizontal) or (self.isVertical and dire.isVertical)

    def isSameSide(self:'Direction', dire : 'Direction') -> bool:
        return self.changeDirection() is dire
...
class UnOrderedLattice(Lattice):
    ...
    @property
    def dimension(self:'UnOrderedLattice') -> int:
        return 2
...
class OrderedLattice(Lattice):
    ...
    @property
    def dimension(self:'OrderedLattice') -> int:
        return 2


class UnOrderedLattice1D(Lattice):

    def __init__(self: 'UnOrderedLattice1D', size: int = 0, seed:tp.Any = 0) -> None:
        super().__init__(size, seed)


    @property
    def dimension(self: 'UnOrderedLattice1D') -> int:
        return 1

    def getAvailableDirections(self: 'UnOrderedLattice1D', x: int, y:int) -> tp.List['Direction']:
        res = []
        for dire in Direction:
            if(dire.isHorizontal):
                if(self.canGoTo(x + dire.offset, y)):
